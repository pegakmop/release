...
function parseControlFields(content) {
  ...
  for (const line of lines) {
    // Пустая строка - сохраняем текущее поле
    if (!line.trim()) {
    ...
    // Продолжение многострочного поля (начинается с пробела или таба)
    if (/^[ \t]/.test(line)) {
      if (currentKey) {
  ...
  // Сохраняем последнее поле (важно для полей в конце файла!)
  if (currentKey && currentValue) {
    result[currentKey] = currentValue.trim();
  }
...
function extractControlFromIpk(ipkPath) {
  ...
  try {
    ...
    const parsed = parseControlFields(controlContent);
    
    // ОТЛАДКА ДЛЯ NEOFIT (можно убрать после проверки)
    if (parsed.Package === 'neofit') {
      const debugPath = path.join(repoRoot, 'DEBUG_neofit.log');
      const debugInfo = `
========================================
IPK: ${ipkPath}
...
Depends found: ${parsed.Depends ? `YES: "${parsed.Depends}"` : 'NO (undefined)'}
========================================
...
function generatePackagesFiles(dir, relPath) {
  ...
  for (const entry of finalPackages) {
    ...
    const block = [
      ...
      control.Depends ? `Depends: ${control.Depends}` : null,
