...
run_with_animation() {
  ...
  echo "$msg"
  ("$@" >"$logfile" 2>&1) &
  pid=$!; tail -n +1 -f "$logfile" & tpid=$!
  while kill -0 $pid 2>/dev/null; do
    printf "\r[%c] " "${spin:$((i++%4)):1}"
    usleep 100000
  done
  wait $pid; status=$?
  kill $tpid 2>/dev/null; rm -f "$logfile"
  if [ $status -eq 0 ]; then
...
{ \"name\":\"HydraRoute Neo\",\"short_name\":\"hr neo\",\"start_url\":\"/\",\"display\":\"standalone\",\"background_color\":\"#1b2434\",\"theme_color\":\"#fff\",\"orientation\":\"any\",\"prefer_related_applications\":false,\"icons\":[{\"src\":\"180x180.png\",\"sizes\":\"180x180\",\"type\":\"image/png\"},{\"src\":\"apple-touch-icon.png\",\"sizes\":\"180x180\",\"type\":\"image/png\"}]}
EOF"
...
if(\$rc!==false){
  ...
  \$notice='<p class=\"error\">⚠️ Не удалось загрузить инфу.</p>';
}
?>
<!DOCTYPE html><html lang=\"ru\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0\"><title>HRNeo Обновление</title><style>body{...}</style></head><body><div class=\"update-box\"><?php if(\$message):?><div class=\"message\"><?=htmlspecialchars(\$message)?></div><?php endif;?><?=\$notice?></div></body></html>
EOF"

  run_with_animation "Загрузка 180x180.png" curl -s -L -o "$HR/180x180.png" https://raw.githubusercontent.com/pegakmop/hrneo/refs/heads/main/opt/share/www/hrneo/180x180.png
  run_with_animation "Загрузка apple-touch-icon.png" curl -s -L -o "$HR/apple-touch-icon.png" https://raw.githubusercontent.com/pegakmop/hrneo/refs/heads/main/opt/share/www/hrneo/apple-touch-icon.png
  run_with_animation "Настройка Lighttpd" sh -c "cat >\"$CF\" << 'EOF'\n\$SERVER[\"socket\"]==\":88\"{server.document-root=\"/opt/share/www/\";server.modules+=(\"mod_cgi\");cgi.assign=(\".php\"=>\"/opt/bin/php8-cgi\");setenv.set-environment=(\"PATH\"=>\"/opt/bin:/usr/bin:/bin\");index-file.names=(\"index.php\");}\nEOF" && /opt/etc/init.d/S80lighttpd enable && /opt/etc/init.d/S80lighttpd restart
}
...
echo ""; echo "Установить/обновить пакет ('hydraroute' или 'hrneo')? (y/n):"
read a < /dev/tty
if [ "$a" = "y" ] || [ "$a" = "Y" ]; then
  echo "Введите имя пакета:"; read PACKAGE < /dev/tty
  case "$PACKAGE" in
    hydraroute|hrneo)
      run_with_animation "Установка/обновление $PACKAGE" opkg install --force-upgrade "$PACKAGE"
      if [ "$PACKAGE" = "hrneo" ]; then
        echo ""; echo "Установить Web-интерфейс для HRNeo? (y/n):"; read w < /dev/tty
        if [ "$w" = "y" ] || [ "$w" = "Y" ]; then
          run_with_animation "Установка HRNeo WebUI" install_webui
        fi
      fi
      ;;
    *) echo "❗ Пропуск: неверное имя пакета.";;
  esac
else
